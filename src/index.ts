...
export const Config: Schema<Config> = Schema.object({
  messageDeleteTime: Schema.number()
    .description("message delete time(second,0 means no delete)")
    .default(0),
  messagePacking: Schema.boolean()
    .default(false)
    .description("message packing"),
});
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx
    .command("pixelstarships.showRoleInfo <name:text>")
  ...
  ctx.command("pixelstarships.downloadData").action(async (argv) => {
